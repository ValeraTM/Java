...
int double_cmp_up (const void * x1, const void * y1) {
    double x = *(double *)x1;
    double y = *(double *)y1;
...
int double_cmp_down (const void * x1, const void * y1) {
    double x = *(double *)x1;
    double y = *(double *)y1;
...
int main (int argc, char * argv[]) {
    ...
    printf("Reading...\n");
    if (gradation == 1) {
        read(head, data, sizeof(double), read_double, write_double, double_cmp_up);
    }
    else {
        read(head, data, sizeof(double), read_double, write_double, double_cmp_down);
    }
    ...
    start = clock();
    if (gradation == 1) {
        merge(head, sizeof(double), scan_double, write_double, double_cmp_up);
    }
    else {
        merge(head, sizeof(double), scan_double, write_double, double_cmp_down);
    }
    finish = clock();
    printf("Passed %f seconds\n", ((float)(finish - start)) / CLOCKS_PER_SEC);
